...
extern "C" {
    void nvme_tcp_driver_top(
        ...
        // from network stream
        hls::stream<DATA_WIDTH> &nvme_tcp_rxdata, // receive
        hls::stream<DATA_WIDTH> &nvme_tcp_txdata, // transmit

        // admin queue address
        nvme_cqe_t *admin_cq_base_addr, // CQ base address
        nvme_io_command_t *admin_sq_base_addr, // SQ base address
        ...
        hls::stream<struct request_packet> &nvme_response_stream,
    )
    {
        ...
        #pragma HLS INTERFACE m_axi port=nvme_request_stream
